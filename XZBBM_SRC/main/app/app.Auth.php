...
Class Auth extends Xzbbm{
	...
	public function Reg(){
		...
		if($u['ccode'] > 0){
			//该用户已存在
			echo json_encode(array('rs' => 4));
			exit;
		}elseif(empty($u['userid'])){
			$this->_db->insert('pd_users',$user);
		}else{
			$this->_db->update('pd_users',array('ucode' => $user['ucode'] ,'ccode' => $user['ccode'] ,'pay_account' => $user['pay_account'],'password' => $user['password']),"userid = {$u['userid']}");
		}
	...
	public function Login(){
		...
		if(!empty($u)){
			...
			$this->_db->update('pd_users',array('last_login_ip' => $u['ip'],'last_login_time' => TIMESTAMP),'userid = '.$u['userid']);
			//登陆成功
			echo json_encode(array('rs' => 0,'info' => $u));
		}else{
			echo json_encode(array('rs' => 3));
		}
	}
