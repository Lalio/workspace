...
function load_app ($appName = '', $doMethod = '')
{
    ...
    $_REQUEST[DO_METHOD] = @$_REQUEST[DO_METHOD] ? ucfirst(
            $_REQUEST[DO_METHOD]) : DEFAULT_DO_METHOD;
    ...
    if (strpos($_REQUEST[DO_METHOD], '_') !== false) {
        ...
        $_REQUEST[DO_METHOD] = str_replace(' ', '', 
                ucwords($_REQUEST[DO_METHOD]));
    }
...
function file_type ($filename)
{
    ...
    $fileCode = intval(
            $strInfo['chars1'] . $strInfo['chars2'] . $strInfo['chars3'] .
                     $strInfo['chars4']);
...
function getzipinfo ($path, $file_size)
{
    $zip = zip_open($path);
    if ($zip) {
        $str = "<h1>这是一份ZIP压缩格式的资料，压缩包内包含如下</h1>";
        $i = 0;
        $j = 0;
        ...
        $str .= $j == 0 ? "<h1>共" . $i . "个文件，压缩包大小为" .
                 round(($file_size / 1024), 2) . "KB</h1>" : "<h1>共" . $i .
                 "个文件，" . $j . "个目录，压缩包大小为" . round(($file_size / 1024), 2) .
                 "KB</h1>";
    }
    ...
    foreach ($result as $k => $v) {
        ...
        if (! empty($output[0])) {
            ...
            $str .= '<p><img onerror="this.src=\'http://' . DOMAIN .
                     '/images/chm.png\'" src="http://cdn.xzbbm.cn/web/images/' .
