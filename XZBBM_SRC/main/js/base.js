...
$.extend({'make_vc':function(){
 	var vc_source = "http://"+domain+"/?action=Auth&do=GVerifyCode&headerType=png&ts="+Math.random();
 	$(".vc_span").html("");
...
$.extend({'login':function(formid){
	$.ajax({
		url:"http://"+domain+"/?action=Auth&do=Login",
...
$.extend({'reg':function(formid){
	$.ajax({
		url:"http://"+domain+"/?action=Auth&do=Reg",
        data:$(formid).serialize(),
        type:"post",
        dataType:"json",
        success:function(data){
...
$.extend({'upload_mdf_cmt':function(file_name,file_index){
	...
	var index = file_index.replace(/(^\s*)/g,"");
  	...
  	$.ajax({
  		url:'http://'+domain+'/?do=UploadMdfFileName',
  		data:{file_index:index,file_name:file_name,tag:$('input[name="tag"]:checked').val()},
  		type:"post",
  		dataType:"json",
        success:function(data){
          location.href = 'http://"+domain+"/view/'+index;
        }
    });
...
$.extend({'change_university':function(value,tb){
	$.ajax({
		url:"http://"+domain+"/?action=Index&do=GetUniversities&province="+value,
        type:"get",
        dataType:"json",
        async:false,
        success:function(data){
...
$.extend({'change_college':function(value){
	$.ajax({
		url:"http://"+domain+"/?action=Index&do=GetColleges&university_id="+value,
        type:"get",
        dataType:"json",
        success:function(data){
        	$(".select_col_1").empty();
...
$.extend({'getmore':function(value){
	...
	$(".switch").html('<img style="padding-top: 15px;" src="http://'+domain+'/images/loading1.gif" />');
	
	$.ajax({
		url:"http://"+domain+"/?do=Search&o=i&from=index&func=json&k="+encodeURI(value),
        type:"get",
        dataType:"json",
        success:function(data){
        	$("#news").empty();
            ...
            for(var o in data){ 
            	if(o < 15){
            		$("#news").append('<li><a target="_blank" class="kan_title" href="./view/'+data[o].file_index+'"><img onerror="this.src=\'http://'+domain+'/images/chm.png\'" src="http://cdn.xzbbm.cn/web/images/'+data[o].file_extension+'.png"> '+data[o].file_name+'</a><span><em class="eye">'+(data[o].file_views)+'人看过</em><em class="lie"> '+data[o].profile+'元收益</em></span></li>');
            		$("#news").fadeIn('fast');
            	}
            }
            $(".switch").html('换一换');
	...
	$.ajax({
		url:"http://"+domain+"/?do=Search&o=u&from=index&func=json&k="+encodeURI(value),
        type:"get",
        dataType:"json",
        success:function(data){
        	$("#hots").empty();
            ...
            for(var o in data){ 
            	if(o < 15){
            		$("#hots").append('<li><a target="_blank" class="kan_title" href="./view/'+data[o].file_index+'"><img onerror="this.src=\'http://'+domain+'/images/chm.png\'" src="http://cdn.xzbbm.cn/web/images/'+data[o].file_extension+'.png"> '+data[o].file_name+'</a><span><em class="eye">'+(data[o].file_views)+'人看过</em><em class="lie"> '+data[o].profile+'元收益</em></span></li>');
            		$("#hots").fadeIn('fast');
            	}
            }
        }
...
$.extend({'add_unirank':function(value){
	$.ajax({
		url:"http://"+domain+"/?action=Index&do=GetUniRank&client="+returnCitySN.cname,
        type:"get",
        dataType:"json",
        success:function(data){
        	$(".geo_pro").empty();
...
$('#file_upload').uploadify({
	'swf'   		   : 'http://'+domain+'/etc/uploader/uploadify.swf',
	'uploader'         : 'http://'+domain+'/?do=Upload',
	'fileSizeLimit'    : '15MB',
	'width'            :  600,
	'buttonCursor'     : 'hand',
	'multi'            : false,
	'rollover'         : true,
	'buttonText'       : '请选择不大于15MB的 Office、PDF、RAR/ZIP、CHM、JPG、PNG 格式学术资料类文档上传',
	'fileTypeDesc' 	   : '各类文档类资料',
	'fileTypeExts'     : '*.doc; *.docx; *.xlsx; *.pptx; *.ppt; *.pdf; *.rar; *.zip; *.chm; *.jpg; *.png;',
	'method'           : 'post',
...
$(document).ready( function() {
	...
	setInterval(function(){
		if(false === s_in_action){
			$.ajax({
				url:"http://"+domain+"/?action=Index&do=GetHotWords",
		        type:"get",
		        dataType:"json",
		        success:function(data){
    ...
    $(".sle_pro").click(function(){
    	...
    	$.ajax({
            ...
            success:function(data){
            	...
            	$(".qh_overflow").toggle();
                for(var o in data){ 
                	$(".qh_overflow").append('<a href="http://'+domain+'/?do=SwitchCollege&ucode='+data[o].id+'">'+data[o].name+'</a>');
                }
    ...
    $(".select_pro").click(function(){
    	$.ajax({
    		url:"http://"+domain+"/?action=Index&do=GetColleges&province="+encodeURIComponent($(this).text()),
            type:"get",
            dataType:"json",
            success:function(data){
                ...
                for(var o in data){ 
                	$("#school .college").append('<li><a onclick="location.href=\'http://'+domain+'/?do=SelectCollege&ucode='+data[o].id+'\'" href="javascript:;" title="'+data[o].name+'">'+data[o].name+'</a></li>');
    ...
    $(".ding").click(function(){
    	$.ajax({
    		url:"http://"+domain+"/?action=Index&do=vote&type=ding&id="+$(this).attr("id"),
            type:"get",
            dataType:"json",
            success:function(data){
            	if(data.rs == 0){
    ...
    $(".cai").click(function(){
    	$.ajax({
    		url:"http://"+domain+"/?action=Index&do=vote&type=cai&id="+$(this).attr("id"),
            type:"get",
            dataType:"json",
            success:function(data){
            	if(data.rs == 0){
    ...
    $("#sendmailsbt").click(function(){
    	...
    	$.ajax({
    		url:"http://"+domain+"/?action=Index&do=SendByMail&file_index="+obj.attr("file_index")+"&addr="+obj.val(),
            type:"get",
	        dataType:"jsonp",
	        jsonp: 'callback',
            success:function(data){
	...
	$('#onlineview').click(function(){
		$.fancybox({
			...
			padding			: 0,
			href: 'http://'+domain+'/?do=OnlineView&idf='+$(this).attr('idf'),
		});
	});
    ...
    if($(".related").length){
	    //setInterval(function(){
	    	$.ajax({
	    		url:"http://"+domain+"/?action=Index&do=GetRelatedFile",
	            type:"get",
	            dataType:"json",
	            success:function(data){
	...
	$("#reg_email").blur(function(){
    	...
    	$.ajax({
    		url:"http://"+domain+"/?action=Auth&do=CheckUser&email="+email,
            type:"GET",
            dataType:"json",
            success:function(data){
            	if(data.rs == 2){
            	...
            	if(data.rs == 0){
            		$("#emailcheck").html('<img src="http://'+domain+'/images/dui.jpg" width="18" height="18">');
            	}
            }
        });
    });
	...
	$("#pwd").blur(function(){
		if($(this).attr("value").length > 0){
	    	if($(this).attr("value").length < 6){
	    		...
	    		$("#pwdrs").html('<img src="http://'+domain+'/images/dui.jpg" width="18" height="18">');
	    	}
		}
    });
	...
	$("#confirm_pwd").blur(function(){
		if($(this).attr("value").length > 0){
	    	if($("#pwd").attr("value") != $("#confirm_pwd").attr("value")){
	    		...
	    		$("#pwdcheckrs").html('<img src="http://'+domain+'/images/dui.jpg" width="18" height="18">');
	    	}
		}
    });
	...
	$("#cloudprint").click(function(){
	    	...
	    	$.ajax({
	    		url:"http://"+domain+"/?action=Index&do=CloudPrint&file_id="+file_id,
	            type:"get",
	            dataType:"json",
	            success:function(data){
	            	if(data.rs == 0){
