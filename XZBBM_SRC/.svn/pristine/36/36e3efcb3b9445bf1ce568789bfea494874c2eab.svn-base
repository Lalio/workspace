...
class Index extends Xzbbm{
	...
	public function My() {
		...
		if($this->userinfo){
			...
			header('Location:http://www.xzbbm.cn');
		}
	...
	public function ViewFile() {
		...
		//实时数据
		$rs = $this->_db->rsArray("SELECT profile,in_recycle,has_png,has_swf,has_pdf,good_count,bad_count,file_views,file_downs,has_thumb FROM pd_files WHERE file_id = ".$file_id);
		...
		if(empty($rs) || $this->pageData['in_recycle'] == '1'){
			...
			$this->_db->conn("UPDATE pd_files SET file_views = file_views + 1 WHERE file_id = $file_id");
		}
		...
		$this->pageData['pics'][] = 'http://www.xzbbm.cn/?action=QrCodes&do=GcQr&fid='.$this->pageData[file_id];
		...
		if(is_mobile() || isset($_REQUEST['imei']) || $_REQUEST['from'] == 'm'){
			...
			$this->pre_url = "http://www.xzbbm.cn/view/{$this->pageData['file_index']}?from={$_REQUEST['from']}&imei={$_REQUEST['imei']}&page=".($this->page-1);
			$this->next_url = "http://www.xzbbm.cn/view/{$this->pageData['file_index']}?from={$_REQUEST['from']}&imei={$_REQUEST['imei']}&page=".($this->page+1);			
			if($this->pageData['has_png'] == 0){ //触发统计
	    		...
	    		<p>
		    		<br><br><object type="application/x-shockwave-flash" data="http://www.xzbbm.cn/etc/reader/dewplayer-vol.swf?mp3=http://gt.xzbbm.cn/{$this->pageData['file_store_path']}/{$this->pageData['file_real_name']}.{$this->pageData['file_extension']}" width="100%" height="50" id="dewplayer-vol">
		    			<param name="width" value="100%">
	    				<param name="height" value="50">
	    				<param name="wmode" value="transparent">
		    				<param name="movie" value="http://www.xzbbm.cn/etc/reader/dewplayer-vol.swf?mp3=http://gt.xzbbm.cn/{$this->pageData['file_store_path']}/{$this->pageData['file_real_name']}.{$this->pageData['file_extension']}">
		    		</object><br><br>
	    		</p>
HTMLCODE;
					}elseif(-8 >= $this->pageData['has_swf']){ //连续40秒未能生成判定为版权保护
						if($this->pageData['has_png'] > 0){
							$this->pageData['attention'] = $this->pageData['has_png'] == 1?'<img style="border:1px solid rgb(216, 218, 216);width:90%;margin:5px;" src="http://www.xzbbm.cn/GetFile/'.$this->pageData['file_id'].'/png/'.TIMESTAMP.'/'.sha1(TIMESTAMP.'sNsxCrth13LGsu60').'">':'<img style="border:1px solid rgb(216, 218, 216);width:90%;margin:5px;" src="http://www.xzbbm.cn/GetFile/'.$this->pageData['file_id'].'/png/'.TIMESTAMP.'/'.sha1(TIMESTAMP.'sNsxCrth13LGsu60').'?bit=0">';
			...
			if(in_array($this->pageData['file_extension'], array('jpg','png'))){
				...
				$this->pageData['attention'] .= '<img style="border:1px solid rgb(216, 218, 216);width:90%;margin:5px;" src="http://www.xzbbm.cn/GetFile/'.$this->pageData['file_id'].'/file/'.TIMESTAMP.'/'.sha1(TIMESTAMP.'sNsxCrth13LGsu60').'">';
			...
			if($this->pageData['has_swf'] == 0){
				if(in_array($this->pageData['file_extension'], array('wma','exe','chm','txt','cpp'))){
					...
					$this->pageData['attention'] = $this->pageData['file_extension'] == 'zip'?getzipinfo($file_path,$this->pageData['file_size']):getrarinfo($file_path,$this->pageData['file_size']);
					if($this->pageData['has_png'] > 0){ //随手拍
						$this->pageData['attention'] = '<div class="nd_rotate" style="text-align: center">';
						$this->pageData['attention'] .= $this->pageData['has_png']==1?'<img style="border:1px solid rgb(216, 218, 216);width:90%;margin:5px;" src="http://www.xzbbm.cn/GetFile/'.$this->pageData['file_id'].'/png/'.TIMESTAMP.'/'.sha1(TIMESTAMP.'sNsxCrth13LGsu60').'">':'<img style="border:1px solid rgb(216, 218, 216);width:90%;margin:5px;" src="http://www.xzbbm.cn/GetFile/'.$this->pageData['file_id'].'/png/'.TIMESTAMP.'/'.sha1(TIMESTAMP.'sNsxCrth13LGsu60').'/0">';
						$this->pageData['attention'] .= '</div>';
						$this->pageData['attention'] .= '<p>%>_<% 基于版权保护规则，当前资料仅提供部分页面预览，请通过APP或发送功能获取完整资料。';
					}
	    		...
	    		<p>{$this->pageData['file_description']}</p>
	    		<p>
		    		<object type="application/x-shockwave-flash" data="http://www.xzbbm.cn/etc/reader/dewplayer-vol.swf?mp3=http://gt.xzbbm.cn/{$this->pageData['file_store_path']}/{$this->pageData['file_real_name']}.{$this->pageData['file_extension']}" width="500" height="50" id="dewplayer-vol">
...
HTMLCODE;
					...
					$refreshtime = 10000;
					$this->pageData['attention'] = <<<HTMLCODE
	    		<p align="center"><img src="http://cdn.xzbbm.cn/web/images/gc_loading.gif"><br><br><strong>资料正在玩命转换中，大概需要十秒钟...</strong></p>
...
HTMLCODE;
	...
	public function Upload() {
		...
		if(TIMESTAMP - $_REQUEST['timestamp'] > 500 ||$_REQUEST['token'] != md5('2zwep62GnVv08Z5W9GGc'.$_REQUEST['timestamp'])){
			exit("你无权通过此接口上传！\nC_tk:$_REQUEST[token] \nC_ts:$_REQUEST[timestamp] \nS_tk:".md5('2zwep62GnVv08Z5W9GGc'.$_REQUEST['timestamp']));
			...
			$uinfo = json_decode($_REQUEST['uinfo'],true);
			if(empty($uinfo)){
	...
	public function Index() {
		...
		$this->pageData['kan'] = $this->GetData("SELECT $select FROM pd_files WHERE in_recycle = 0 AND has_thumb > 0 GROUP BY file_name ORDER BY rand() limit 0,20");
	...
	public function GetUniRank() {
		...
		foreach($rs as $k=>$v){
			$rt['ctc'] .= "<a href='javascript:;'><img height='27px' src='http://cdn.xzbbm.cn/images/sicons/{$v['sicon_id']}.png' onerror='this.src=\"http://cdn.xzbbm.cn/images/sicons/0.png\"'> {$v['name']}({$v['total_files']})</a>";
		}
	...
	public function GetFile(){
		...
		if(in_array($ext,array('jpg','png','pdf'))){
			...
			header("Location:http://oss.xzbbm.cn/{$rs['file_real_name']}-s-$bit.png");
		}
	}
	...
	public function UploadMdfFileName(){
		...
		if($_SERVER['HTTP_ORIGIN'] != 'http://www.xzbbm.cn'){
			exit('拒绝服务！');
		}
		...
		echo json_encode(array('url' => 'http://www.xzbbm.cn/view/'.$file_index));
	}
...
HTML;
		...
		foreach ($rs as $k => $v){
			if($k%2){
				echo <<<HTML
			<tr>
				<td><img height="125px" width="125px" src="http://www.xzbbm.cn/?action=QrCode&do=Liantu&file_id={$v['file_id']}"></td>
