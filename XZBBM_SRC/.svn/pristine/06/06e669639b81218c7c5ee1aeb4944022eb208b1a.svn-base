 ...
 /**
    ...
    function SetAdmin($pagesize=20,$total,$current=false,$target='_self', $rewrite=false) {
        ...
        if ($this->tpage >= 1) {
            ...
            if ($current>1) {
                $this->output.='<a href="./index.php?'.($this->varstr).$gue2.$this->pvar.$gue3.'1'.'" title="首页" target="'.$target.'">首页  </a>';
            }
            if ($current>1) {
                $this->output.='<a href="./index.php?'.($this->varstr).$gue2.$this->pvar.$gue3.($current-1).'" title="上页" target="'.$target.'">上页 </a>';
            }
            ...
            for ($i=$start; $i<=$end; $i++) {
                if ($current==$i) {
                    $this->output.='<span class="current"><a href="javascript:"> <font color="red"><b>'.$i.'</b></font> </a></span>';    //输出当前页数
                } else {
                    $this->output.='<a href="./index.php?'.$this->varstr.$gue2.$this->pvar.$gue3.$i.'" target="'.$target.'">'.$i.'</a>';    //输出页数
                }
            }
            ...
            if ($current<$this->tpage) {
                $this->output.='<a href="./index.php?'.($this->varstr).$gue2.$this->pvar.$gue3.($current+1).'" title="下页" target="'.$target.'">下页 </a>';
            }
            if ($this->tpage > $current) {
                $this->output.='<a href="./index.php?'.($this->varstr).$gue2.$this->pvar.$gue3.$this->tpage.'" title="尾页" target="'.$target.'">尾页</a>';
            }
