...
#}
	 ...
	 */
	public function Tb(){
		$rss = $this->_db->dataArray("SELECT userid,email,phone FROM pd_users WHERE user_name = ''");
		
		foreach($rss as $rs){
	
		    $user_name = substr_replace ( $rs['email'], '****', 3, 3 )?substr_replace ( $rs['email'], '****', 3, 3 ):substr_replace ( $rs['phone'], '****', 3, 3 );
			var_dump($this->_db->update('pd_users',array('user_name' => $user_name),"userid = $rs[userid]"));
		}
