...
Class MadsService{
	...
	protected $_db_test; //mysql测试环境数据库句柄
	...
	function __construct(){
		...
		Core::InitDb();
		$this->_db = Core::$db['main'];  //当前机
		$this->_db_mads = Core::$db['mads'];  //正式主机
		$this->_db_slave = Core::$db['mads_slave']; //正式从机
		$this->_db_test = Core::$db['mads_test'];  //测试机
		$this->_db_t3 = Core::$db['t3'];  //t3机
		$this->_db_source = Core::$db['mads_dbsource'];  //数据源机
	...
	public function PushEmail($topic,$body,$to,$rid = '',$cc = 'bo.wang3,junfeng.deng'){
		...
		if(strpos($to,",") !== false){
			$to = explode(',', $to);
			foreach($to as $v){
				if(strstr($v,'@renren-inc.com')){
					$mail->AddAddress($v, "");
				}else{
					$mail->AddAddress($v."@renren-inc.com", "");
				}
			}
		}else{
