 ...
 */
//ACCESS_ID
define('OSS_ACCESS_ID', 'mpFFmaUpbVb62R1F');
//ACCESS_KEY
define('OSS_ACCESS_KEY', 'mUcDNjdQSciQnXTxGIeL19jmv5BbSG');
//OSS_BUCKET
define('OSS_BUCKET', 'xzbbm-oss');


//是否记录日志
define('ALI_LOG', FALSE);
//自定义日志路径，如果没有设置，则使用系统默认路径，在./logs/
define('ALI_LOG_PATH','/dev/shm/osslog');
//是否显示LOG输出
...
if(!is_dir('/dev/shm/osslog')){
	mkdir('/dev/shm/osslog',0777);
}

if(!is_dir('/dev/shm/osstmp')){
	mkdir('/dev/shm/osstmp',0777);
}