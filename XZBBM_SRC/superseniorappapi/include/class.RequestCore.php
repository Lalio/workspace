...
class RequestCore
{
	...
	public function streaming_read_callback($curl_handle, $file_handle, $length)
	{
		...
		if ($this->read_stream_read == 0 && isset($this->seek_position) && $this->seek_position !== ftell($this->read_stream))
		{
			...
			{
				throw new JException('The stream does not support seeking and is either not at the requested position or the position is unknown.');
			}
		}
		...
		switch ($this->method)
		{
			case self::HTTP_PUT:
				...
				if (isset($this->read_stream))
				{
					...
					{
						throw new JException('The stream size for the streaming upload cannot be determined.');
					}
	...
	public function send_request($parse = false)
	{
		...
		{
			throw new JException('cURL resource: ' . (string) $curl_handle . '; cURL error: ' . curl_error($curl_handle) . ' (' . curl_errno($curl_handle) . ')');
		}
	...
	public function send_multi_request($handles, $opt = null)
	{
		...
		do
		{
			...
			while ($done = curl_multi_info_read($multi_handle))
			{
				...
				if ($done['result'] > 0)
				{
					throw new JException('cURL resource: ' . (string) $done['handle'] . '; cURL error: ' . curl_error($done['handle']) . ' (' . $done['result'] . ')');
				}
