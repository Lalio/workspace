...
class Db {
    ...
    public function Conn($sql, $bind = '') {
        ...
        //SQL
        if ($sql)
            $this->_sql = $this->formatSql ( iconv('utf8' , 'gbk' , $sql), $bind );
     ...
     *
     */
    public function BinInsert($table, $bind, $ext = '') {
     ...
     *
     */
    public function BinRsArray($sql = '', $bind = '') {
    	if ($sql)
    		$this->Conn ( $sql, $bind );
    	if ($this->_rs) {
    ...
    public function RsArray($sql = '', $bind = '') {
        ...
        if ($this->_rs) {
            $rs = mysql_fetch_assoc ( $this->_rs );
            return is_utf8($rs) ? $rs:gbk2utf8($rs); //判断编码并返回正确的编码格式
        } else {
            return false;
        }
    }
