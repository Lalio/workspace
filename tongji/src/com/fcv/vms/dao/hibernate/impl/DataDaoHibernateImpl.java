...
//					+ " order by time asc limit 1";
			String sql = "SELECT time FROM " + tableNameString
					+ " where time >= '2015-07-01 00:00:00' order by time asc limit 1";
...
//		final String sql = "select distinct(vin) from data where time >= '2015-07-01 00:00:00'";
		final String sql = "select vin from reg ";
		...
		for (String string : list) {
			if (string.matches(regex)) {//合法的VIN是17位的并且V+数字结尾。用正则表达式过滤掉垃圾数据
	...
	public void getAllDatas(long begin,long end) {
		int num = 0;
		final String sql = "select * from data  order by time asc limit "+begin+","+end;
	...
	public void getAllDatas(String vinString) {
		String tableNameString = "z_param_" + vinString.toLowerCase();
