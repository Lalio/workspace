...
//		System.err.println("isHave    "+result);
	...
	public int getWarnNum(String vin, Date today2) {
		...
		String sql = "SELECT count(*) as num FROM warnstat_new where vin = '"+vin+"'  and  stime >= '" + sdf.format(begintime)
	...
	@Override
	public Map<String , WarnStat> getLastWarn(String vin) {
		Map<String , WarnStat> result = new HashMap<String,WarnStat>();
		Session session = null ;
		Transaction transaction = null;
		try {
			String sql = "SELECT * FROM warnstat_new where id in ( SELECT MAX(id) FROM warnstat_new WHERE vin =  '"+vin+"' GROUP BY warnName )";
			session = getHibernateTemplate().getSessionFactory().openSession();
			transaction = session.beginTransaction();
			List<WarnStat> list = session.createSQLQuery(sql).addEntity(WarnStat.class).list();
			for (WarnStat warnStat : list) {
				result.put(warnStat.getWarnName(),warnStat);
			}
		} catch (Exception e) {
			System.out.println(vin + " has got exception");
